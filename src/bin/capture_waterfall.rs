...
use sdaa_data::{
    ddc::N_PT_PER_FRAME, payload::Payload, pipeline::{pkt_wf, recv_pkt}, utils::slice_as_u8, RAW_SAMP_RATE
};
...
fn main() {
    ...
    println!("dt={dt}={}/{} dt per iter={}", args.nch*2*args.nint, RAW_SAMP_RATE, dt_per_iter);
    for _i in 0.. {
        ...
        args.outname.as_ref().map(|outname|{
            std::fs::OpenOptions::new().append(true).create(true).open(outname).unwrap()
        }).iter_mut().for_each(|f| {
